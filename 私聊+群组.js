...
async function sendAdminInterface(userId, targetChatId, threadId, mode) {
  ...
  const nicknamePlain = userInfo
    ? `${userInfo.first_name || ''} ${userInfo.last_name || ''}`.trim() || `UID:${userId}`
    : `UID:${userId}`;
  const nicknameEsc = escapeMarkdown(nicknamePlain);
  const usernameRaw = userInfo?.username ? `@${userInfo.username}` : null;
  const usernameEsc = usernameRaw ? escapeMarkdown(usernameRaw) : null;
